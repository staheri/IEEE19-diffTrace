...
\hl{As proof of concept, we have preliminary results from using ParLOT tracing mechanism to distinguish between runs, as studied in} \citep{cstg}. \hl{Using PMPI profiler, we have injected different bugs to MPI-related functions to ILCS} \cite{ilcs} \hl{source-code, a scalable parallelization framework for iterative local searches. We executed ParLOT on top of executions of buggy and bug-free versions of ILCS and collected traces.
Since ParLOT's traces maintain the order of function calls, we were able to split traces at multiple points of interest and by feeding different chunks of traces to a data structure Concept Lattice} \cite{clbook} \cite{clconst}. 
\hl{We used Concept Lattice properties for trace analysis, e.g., similarity measurements between different processes or trace chunks and inter-process workflow divergence detection. We could narrow down our search space to locate the cause of abnormal behavior of buggy version of ILCS. The tracing methods implemented in ParLOT not only can be used for debugging purposes, but also produce valuable information for performance evaluation and detecting bottlenecks. However, the focus of this paper is on evaluation of tracing and compression mechanism.
