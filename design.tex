...
%
\hl{Basic code blocks within program executables dynamically instrumented before being executed and collected data getting compressed on-the-fly during runtime. 
%
Our experimental results in} \S\ref{sec:results} \hl{ highlights why \textit{compression} is essential to make our approach work.
%
We used ParLOT to record a unique 16-bit identifier for every function call and return.
%
Tracing just this small amount of information without compression when running the Mantevo miniapps~}\cite{mantevo}\hl{ on Stampede 1 resulted in about 2 MB/s of data per core on average.
%
...
%
Enabling ParLOT's compression algorithm reduced the emitted trace data by a factor of 100 on average, a ratio that is quite stable w.r.t scaling, making it possible to trace full-scale programs while leaving over 98\% of the I/O bandwidth to the application. Therefore, it is clear that ParLOT also works for codes with higher required bandwidth than the ones we tested.
}
\begin{figure}[!t]
\centering
